...
- VNC 远程桌面:无需打开 SSH/终端会话，直接填写 VNC 主机和端口并在浏览器内连接
...
1. 无需先打开 SSH 或终端会话，直接点击工具栏 **VNC**；
2. 填写 VNC 服务器的 IP 地址或主机名，以及 VNC 端口，例如显示器 `:1` 通常使用 `5901`，`:2` 使用 `5902`；
3. 输入 VNC 密码并点击 **连接**。密码只用于本次连接，不会保存；
4. VNC 采用独立 TCP 直连，请确保目标服务监听可访问的网卡地址，并允许本机通过防火墙访问对应端口；
5. 面板支持缩放、只读模式、Ctrl+Alt+Del 和全屏。关闭 VNC 面板不会影响已经打开的终端会话。
