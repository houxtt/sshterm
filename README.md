...
- VNC 远程桌面:复用当前 SSH 会话访问远端 `127.0.0.1:5901` 等 VNC 服务，浏览器内嵌显示，无需暴露 VNC 端口
...
1. 先通过 SSH 连接目标主机，并确保远端 VNC 服务已经启动，例如显示器 `:1` 通常监听 `127.0.0.1:5901`；
2. 点击工具栏 **VNC**，端口填写 `5901`（显示器 `:2` 填写 `5902`）；
3. 输入 VNC 密码并点击 **连接**。密码只用于本次连接，不会保存；
4. VNC 流量通过现有 SSH 连接转发，不需要在服务器防火墙开放 5901，也不需要本机安装 VNC Viewer；
5. 面板支持缩放、只读模式、Ctrl+Alt+Del 和全屏。关闭 VNC 面板会断开桌面，但不会断开 SSH 终端。
...
- 使用 `launcher.vbs` 时，关闭浏览器不会停止后台服务，再次启动会使用仍在后台内存中的凭据恢复会话；如需彻底停止，可在工具栏选择“全部关闭”后结束后台 Node 进程或注销 Windows；
