...
(async () => {
  ...
  const ok = autoOk && listCount >= 1 && execOk;
  console.log(`\n=== 汇总: ${ok ? '✅ 命令保存/执行/自动脚本正常' : '❌'} ===`);

  // 清理 (删除测试命令)
  await page.evaluate(() => localStorage.removeItem('sshterm.commands'));
  try {
